...
class LorentzForce(Scene):
    def construct(self):
        ...
        magnetic_force_arrow = Arrow(start = charged_particle.get_center(), end = charged_particle.get_center() + [0,0,-0.5] , color = BLUE).rotate(PI/2,axis = RIGHT)
        ...
        lorentz_force_arrow = Arrow(start = charged_particle.get_center(), end = charged_particle.get_center() + [-4, 0.5, -0.5], color=GREEN).rotate(PI/2, axis=RIGHT)  # Corrected direction here
